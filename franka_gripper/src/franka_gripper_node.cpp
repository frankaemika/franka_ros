...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ROS_ERROR_STREAM("franka_gripper_node: NetworkException creating gripper_server: " << ex.what());
        return -1;
    } catch (franka::IncompatibleVersionException& ex) {
        ROS_ERROR_STREAM("franka_gripper_node: IncompatibleVersionException creating gripper_server: " << ex.what());
        return -1;
    } catch (franka::ProtocolException& ex) {
        ROS_ERROR_STREAM("franka_gripper_node: ProtocolException creating gripper_server: " << ex.what());
        return -1;
    }
