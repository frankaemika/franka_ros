...
TEST(FrankaHWTests, InterfacesWorkForReadAndCommand) {
  std::unique_ptr<franka_hw::FrankaHW> robotptr(new FrankaHW(joint_names, arm_id, ros::NodeHandle()));
...
TEST(FrankaHWTests, JointLimitInterfacesEnforceLimitsOnCommands) {
  std::unique_ptr<franka_hw::FrankaHW> robot_ptr(new FrankaHW(joint_names, arm_id, ros::NodeHandle()));
