...
franka_hw::FrankaHW::FrankaHW(const std::vector<std::string> &joint_names,
                              const std::string &ip, const ros::NodeHandle &nh)
    : robot_(ip.c_str()),
...

{
    for (size_t i = 0; i < joint_name_.size(); ++i) {
        hardware_interface::JointStateHandle joint_handle1(joint_name_[i],
                                                         &robot_state_.q[i],
                                                         &robot_state_.dq[i],
                                                         &robot_state_.tau_J[i]);
        joint_state_interface_.registerHandle(joint_handle1);
        hardware_interface::FrankaJointStateHandle joint_handle2(joint_name_[i],
                                                               robot_state_.q[i],
                                                               robot_state_.dq[i],
                                                               robot_state_.tau_J[i],
                                                               robot_state_.q_d[i],
                                                               robot_state_.q_start[i],
                                                               robot_state_.dtau_J[i],
    ...
    hardware_interface::FrankaCartesianStateHandle cartesian_handle(
                ...
                robot_state_.cartesian_collision,
                robot_state_.cartesian_contact,
                robot_state_.O_F_ext_hat_EE,
                robot_state_.EE_F_ext_hat_EE,
    ...
    if (publisher_franka_states_.trylock()) {
        ...
        publisher_franka_states_.msg_.EE_F_ext_hat_EE.resize(robot_state_.EE_F_ext_hat_EE.size());
        publisher_franka_states_.msg_.elbow_start.resize(robot_state_.elbow_start.size());
        ...
        publisher_franka_states_.msg_.O_F_ext_hat_EE.resize(robot_state_.O_F_ext_hat_EE.size());
...
void franka_hw::FrankaHW::publishFrankaStates() {
    if (publisher_franka_states_.trylock()) {
        for (size_t i = 0; i < robot_state_.cartesian_collision.size(); ++i) {
            ...
            publisher_franka_states_.msg_.EE_F_ext_hat_EE[i] = robot_state_.EE_F_ext_hat_EE[i];
            publisher_franka_states_.msg_.O_F_ext_hat_EE[i] = robot_state_.O_F_ext_hat_EE[i];
        }
        ...
        for (size_t i = 0; i < robot_state_.elbow_start.size(); ++i) {
            publisher_franka_states_.msg_.elbow_start[i] = robot_state_.elbow_start[i];
        }
        ...
        ROS_WARN("could not lock franka_states for publishing, missed %i of %i",
                 int(missed_pulishes_franka_states_), int(sequence_number_franka_states_));
        sequence_number_franka_states_++;  // package loss can be detected when seq_nr jumps
    }
}
...
void franka_hw::FrankaHW::publishJointStates() {
    if (publisher_joint_states_.trylock()) {
        ...
        ROS_WARN("could not lock joint_states for publishing, missed %i of %i",
                 int(missed_pulishes_joint_states_), int(sequence_number_joint_states_));
        sequence_number_joint_states_++;  // package loss can be detected when seq_nr jumps
    }
}
