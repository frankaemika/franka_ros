...
void franka_hw::RealTimeTfPublisher::setTransform(const geometry_msgs::TransformStamped & transform) {
  publisher_tf_.msg_.transforms.resize(1);
  publisher_tf_.msg_.transforms[0] = transform;
}

void franka_hw::RealTimeTfPublisher::setTransform(const tf::StampedTransform & transform) {
...
void franka_hw::RealTimeTfPublisher::setTransform(const std::array<double, 16> & transform,
  ...
  tf::Matrix3x3 rotation(transform[0], transform[4], transform[8],
